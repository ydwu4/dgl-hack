...
class EglAPPNPConv(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        with self.cm.zoomIn(namespace=[self, th], graph=graph, node_feats={'feat':feat, 'feat_0':feat_0, 'norm':norm}) as v:
            for _ in range(self._k):
                feat = sum([nb.feat * nb.norm for nb in v.innbs])
                feat = feat * v.norm
                feat = (1 - self._alpha) * feat + self._alpha * v.feat_0
                self.cm.update_allnode({'feat': feat})
            self.cm.collect_output(v.feat)
        return self.cm.zoomOut() 
